---
title: "Simple Linear Regression Homework - John Hios"
author: "John Hios"
date: "09/05/2022"
output:
  html_document:
...
```
We would say that the data are very strongly positively correlated as per the scaling suggested by  J. D. Evans (1996).

<br>

#### Perform a simple linear regression using `actual_length` as the dependent variable, and `estimated_length` as the independent variable. 
...
summary(model)
```
  + The intercept has a $p$-value much larger than a significance level of $\alpha$ = 0.05, therefore we fail to reject the hypothesis that the intercept is zero ($H_0: intercept = 0$).

  + The slope $p$-value is less than the $\alpha$ = 0.05 significance level, therefore we can be reasonably certain that the coefficient is significantly different from zero.

  + The coefficient of determination is $r^2$ = 0.6401. This means that the simple linear regression model explains the 64.01% of the variation in ``actual_length``.  

<br>

## Is the relationship statistically significant? 

Remember, to assess this you need to check the $p$-value of the regression coefficient (or slope/gradient). But you should first check the regression diagnostic plots to see if the $p$-value will be reliable (don't worry about any outlier points you see in the diagnostic plots, we'll return to them in the extension).
...
The calculated $p$-value is  reliable, as:

  + The ``residuals vs fitted`` plot has residuals randomly scattered around  0 (apart from outlier values 5 and 18), whilst the smooth blue line is close to zero.

  + The ``Q-Q`` plot suggests that the data are normally distributed aside from 2 outlier values (5 and 18).

  +  The ``scale-location``plot shows that the residuals occur in a band of fixed width above the x-axis with the smooth blue line staying close to +1. 
