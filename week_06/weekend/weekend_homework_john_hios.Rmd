...
**Ans 3. **
Let’s start by setting a significance level aplha=0.05, amd the hypothesis test (one-tailed): 
...
Judging from Ha we see that this is a one-tailed test because alternative is that the difference is greater than zero i.e. right direction.
```{r}
null_distribution %>%
  visualise() +
  shade_p_value(obs_stat = observed_stat, direction = "right")
```
We see from the visualisation that the observed statistic is not on the tail of our null distribution and so it looks likely we will not be rejecting H0. To confirm this, let’s calculate the p-value.
```{r}
p_value <- null_distribution %>%
...
p_value
```
Since our p-value is greater than alpha=0.05 we fail to reject H0. There is not enough evidence to say that the omnivores sleep on average more hrs than the herbivores.
...
Now generate the null distribution. The value π0 we put in here should be the one assuming H0 is true.
```{r}
...
p_value
```
Since our p-value is less than alpha = 0.05 we reject H0. Based on the data, the proportion of domesticated mammals is greater than 5%.
...
We want to investigate whether the population proportion that knows about the coffee shop is significantly larger from 40%.
